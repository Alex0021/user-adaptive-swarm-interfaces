from eye_tracker.calibration import invoke_eyetracker_calibration_manager, save_latest_calibration
...
def main():
    ...
    eyetracker.subscribe_to(tr.EYETRACKER_GAZE_DATA, streamer.gaze_data_callback, as_dictionary=True)
    ...
    while True:
        try:
            ...
            for frame in ANIMATION_FRAMES:
                print(f"\r{frame} Publishing gaze data...", end="")
                time.sleep(0.5)
        except KeyboardInterrupt:
            print("-" * 30)
            logger.info("Eye Tracker Service stopping.")
            streamer.stop_stream()
            eyetracker.unsubscribe_from(tr.EYETRACKER_GAZE_DATA, streamer.gaze_data_callback)
            break
